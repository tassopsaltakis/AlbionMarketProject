...
export async function marketRequest<T>(query: string): Promise<Envelope<T>> {
  ...
  const stored = memory.get(url);
  const ttl =
    params.get('kind') && params.get('kind') !== 'prices' ? 300000 : 15000;
  if (stored && Date.now() - stored.time < ttl)
    return { ...stored.value, cached: true } as Envelope<T>;
  if (inFlight.has(url)) return inFlight.get(url) as Promise<Envelope<T>>;
  const task = (async () => {
    try {
      let result: Envelope<T>;
      if (runtime().static) result = await browserSource<T>(params);
      else {
        const res = await fetch(url, { signal: AbortSignal.timeout(55000) });
        result = (await res.json()) as Envelope<T>;
